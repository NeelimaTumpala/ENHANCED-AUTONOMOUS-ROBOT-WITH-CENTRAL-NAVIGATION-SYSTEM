...
Adafruit_DCMotor *motor_red = AFMS.getMotor(2); 
Adafruit_DCMotor *motor_wht = AFMS.getMotor(1);
...
char pass[] = "TechGarage@127";        // your network password (use for WPA, or use as key for WEP)
...
void grab() {
  ...
  myservo.write(180);    // servo moves down

delay(300);
  
  myservo.write(60);    // servo moves up
  delay(300);
}
        ...
        if (httpReq.endOfRequest()) {
          if(httpReq.uri[1] == 'm') {
            ...
            for(int i = 1; i <= httpReq.paramCount; i++){
              ...
              else if(distance < 100) move_speed = 140;
              else move_speed = 170;
              ...
              if(!enabled) release_motors();
                ...
                if(angle <= 0.1 && angle >= -0.1) move_forward();
