...
def draw_bot2(frame, pos, dirn):
	cv2.line(frame, tuple(pos), tuple(pos + dirn), (0, 0, 255), 1) 

def draw_rts2(frame, pos, rts, angle, dist):
	for (i, rt) in enumerate(rts):
		if i == 0:
			cv2.line(frame, tuple(rt.a), tuple(rt.b), (0, 255, 0), 1) 
		if i != 0:
			cv2.line(frame, tuple(rt.a), tuple(rt.b), (0, 0, 255), 1)
def draw_bot3(frame, pos, dirn):
	cv2.line(frame, tuple(pos), tuple(pos + dirn), (0, 0, 255), 1) 

def draw_rts3(frame, pos, rts, angle, dist):
	for (i, rt) in enumerate(rts):
		if i == 0:
			cv2.line(frame, tuple(rt.a), tuple(rt.b), (0, 255, 0), 1) 
		if i != 0:
			cv2.line(frame, tuple(rt.a), tuple(rt.b), (0, 0, 255), 1)
def draw_bot4(frame, pos, dirn):
	cv2.line(frame, tuple(pos), tuple(pos + dirn), (0, 0, 255), 1) 

def draw_rts4(frame, pos, rts, angle, dist):
	for (i, rt) in enumerate(rts):
		if i == 0:
			cv2.line(frame, tuple(rt.a), tuple(rt.b), (0, 255, 0), 1) 
		if i != 0:
...
def draw_pts3(frame, pts):
	for pt in pts:
		cv2.drawMarker(frame, tuple(pt), (0, 255, 0), cv2.MARKER_TILTED_CROSS, 10, 1, 8)
		cv2.putText(frame, "({},{})".format(pt[0], pt[1]), tuple(pt - 20),
			cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

def draw_pts4(frame, pts):
	for pt in pts:
		cv2.drawMarker(frame, tuple(pt), (0, 255, 0), cv2.MARKER_TILTED_CROSS, 10, 1, 8)
		cv2.putText(frame, "({},{})".format(pt[0], pt[1]), tuple(pt - 20),
			cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
...
def display_msg(frame, msg, color):
	cv2.putText(frame, msg, (410, 450),
		cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)



def display_msg2(frame, msg, color):
	cv2.putText(frame, msg, (410, 30),
		cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)

def display_msg3(frame, msg, color):
	cv2.putText(frame, msg, (30, 30),
		cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)



def display_msg4(frame, msg, color):
	cv2.putText(frame, msg, (30, 450),
		cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
